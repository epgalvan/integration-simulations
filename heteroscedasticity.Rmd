...
Import the workspace established at the top levels script and define the function that will make the data more heteroscedastic.
...
plottingInfoAll = data.frame(Outcome = rep(c("Incorrect & N.S.", 
                             ...
                             Count = 0,
                             factor = rep(c(0, 0.25, 0.5, 0.75, 1), each = 4))

sampleSize = 60
nTrials = 60

Factors = c(0, 0.25, 0.5, 0.75, 1)
...
for (factor in Factors){
  ...
  for (i in 1:nrow(df)){
    ...
    data$Transferred = warp(preds, data$Endowment, factor)
...
ggplot(data = final, aes(x = Proportion, y = factor, group = Outcome,
  ...
  labs(x = "Proportion", y = "Variance Warp Factor", 
       fill = "Result",
       label = NULL) + 
...
```{r}
save.image(file = "C:/Users/DELL/Documents/integration-simulations/heteroscedasticity.RData")
```
